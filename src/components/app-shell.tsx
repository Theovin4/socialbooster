...
export function AppShell({ admin = false, children }: { admin?: boolean; children: React.ReactNode }) {
  ...
  const navigation = <><div className="portal-brand"><Logo /><span className="portal-role">{admin ? "Administration" : "Customer portal"}</span></div><nav className="portal-nav" aria-label={admin ? "Admin navigation" : "Customer navigation"}>{links.map(({ label, href, icon: Icon }) => { const active = pathname === href || (href !== (admin ? "/admin" : "/dashboard") && pathname.startsWith(`${href}/`)); return <Link key={href} href={href} onClick={() => setOpen(false)} className={`portal-link${active ? " active" : ""}`}><Icon size={18} strokeWidth={1.8} /><span>{label}</span></Link>; })}</nav><div className="portal-sidebar-footer"><button className="portal-link portal-logout" onClick={logout}><LogOut size={18} /><span>Sign out</span></button></div></>;

  return <div className="portal-layout"><aside className="portal-sidebar">{navigation}</aside><header className="portal-mobile-header"><Logo /><div className="portal-header-actions"><button className="portal-top-logout" onClick={logout} aria-label="Sign out"><LogOut size={18} /><span>Sign out</span></button><button className="icon-button" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu /></button></div></header>{open ? <div className="portal-drawer-layer" role="dialog" aria-modal="true" aria-label="Navigation"><button className="portal-backdrop" onClick={() => setOpen(false)} aria-label="Close navigation" /><aside className="portal-drawer"><button className="icon-button portal-drawer-close" onClick={() => setOpen(false)} aria-label="Close navigation"><X /></button>{navigation}</aside></div> : null}<main className="portal-content"><div className="portal-desktop-actions"><button className="portal-top-logout" onClick={logout}><LogOut size={18} /><span>Sign out</span></button></div>{children}</main></div>;
}
