...
export async function validateSupportFiles(formData: FormData) {
  ...
  if (files.reduce((sum, file) => sum + file.size, 0) > maxTotalBytes) throw new Error("Attachments must be 3 MB or less in total.");
...
export async function storeSupportFiles(ticketId: string, messageId: string, files: Awaited<ReturnType<typeof validateSupportFiles>>) {
  ...
  try {
    for (const { file, bytes } of files) {
      ...
      const path = `support/${ticketId}/${messageId}/${id}-${cleanName}`;
      await bucket.file(path).save(Buffer.from(bytes), { resumable: false, contentType: file.type, metadata: { cacheControl: "private, no-store", metadata: { ticketId, messageId } } });
      stored.push({ id, name: cleanName, contentType: file.type, size: file.size, path });
    }
    return stored;
  } catch (error) {
    await Promise.all(stored.map((attachment) => bucket.file(attachment.path).delete({ ignoreNotFound: true }).catch(() => undefined)));
    throw error;
  }
}
