...
export async function createAndSubmitOrder(input: NewOrder) {
  ...
  const local = await db.runTransaction(async (transaction) => {
    ...
    const customerPriceMinor = Number(serviceCostMinor(BigInt(data.sellingRateMinor), BigInt(input.quantity))), providerCostMinor = Number(serviceCostMinor(BigInt(data.providerRateMinor), BigInt(input.quantity)));
    ...
    if (currency !== (data.providerCurrency || "USD")) throw new Error("Currency conversion is not configured for this wallet");
    ...
    const order = { userId: input.userId, serviceId: input.serviceId, serviceName: data.name, providerServiceId: data.providerServiceId, link: input.link, quantity: input.quantity, currency, sellingRateMinor: data.sellingRateMinor, providerRateMinor: data.providerRateMinor, providerCostMinor, customerPriceMinor, grossProfitMinor: customerPriceMinor - providerCostMinor, marginBps: data.marginBps, refillSupported: data.refillSupported, cancelSupported: data.cancelSupported, status: "submitting", idempotencyKey: input.idempotencyKey, createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() };
