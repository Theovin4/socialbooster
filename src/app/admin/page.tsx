...
export default async function Admin({ searchParams }: { searchParams: Promise<{ period?: string }> }) {
  ...
  try {
    const [services, orders, wallets, transactions, growth] = await Promise.all([db.collection("services").where("active", "==", true).get(), db.collection("orders").limit(250).get(), db.collection("wallets").limit(250).get(), db.collection("walletTransactions").limit(250).get(), customerGrowth(period)]);
    activeServices = services.size;
    pending = orders.docs.filter((doc) => !["completed", "cancelled", "refunded", "failed"].includes(String(doc.get("status") || "").toLowerCase())).length;
    depositsMinor = transactions.docs.filter((doc) => doc.get("type") === "deposit" && doc.get("currency") === "NGN").reduce((total, doc) => total + Number(doc.get("deltaMinor") || 0), 0);
    walletMinor = wallets.docs.filter((doc) => String(doc.get("currency") || "NGN") === "NGN").reduce((total, doc) => total + Number(doc.get("availableMinor") ?? doc.get("balanceMinor") ?? 0), 0);
    customers = growth;
  } catch (error) {
    dataAvailable = false;
    console.error("[admin-dashboard] operational data unavailable", { error: error instanceof Error ? error.message : "Unknown error" });
  }
  ...
  return <AppShell admin>
    {!dataAvailable ? <div className="notice" style={{ marginBottom: 22 }}><strong>Administrator access confirmed.</strong><p className="muted" style={{ marginBottom: 0 }}>Operational totals are temporarily unavailable. Please refresh in a few minutes; no account or payment data has been changed.</p></div> : null}
