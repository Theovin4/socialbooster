...
type Growth = Awaited<ReturnType<typeof customerGrowth>>;
...
async function customerGrowth(period: Period) {
  ...
  const buckets = Array.from({ length: bucketCount }, (_, index) => {
    ...
    return { label: period === "7" ? start.toLocaleDateString("en-NG", { weekday: "short" }) : start.toLocaleDateString("en-NG", { day: "numeric", month: "short" }), count: customers.filter((created) => created >= start && created < end).length };
  });
...
export default async function Admin({ searchParams }: { searchParams: Promise<{ period?: string; activation?: string; sent?: string; skipped?: string; failed?: string }> }) {
  ...
  const db = adminDb();
  let dataAvailable = true, activeServices = 0, pending = 0, depositsMinor = 0, walletMinor = 0;
  let customers: Growth = { total: 0, today: 0, week: 0, selectedCount: 0, growth: 0, buckets: [] };
  const results = await Promise.allSettled([db.collection("services").where("active", "==", true).get(), db.collection("orders").limit(250).get(), db.collection("wallets").limit(250).get(), db.collection("walletTransactions").limit(250).get(), customerGrowth(period)] as const);
  results.forEach((result, index) => { if (result.status === "rejected") { dataAvailable = false; console.error("[admin-dashboard] metric unavailable", { metric: ["services", "orders", "wallets", "transactions", "customers"][index], error: result.reason instanceof Error ? result.reason.message : "Unknown error" }); } });
  const [services, orders, wallets, transactions, growth] = results;
  if (services.status === "fulfilled") activeServices = services.value.size;
  if (orders.status === "fulfilled") pending = orders.value.docs.filter((doc) => !["completed", "cancelled", "refunded", "failed"].includes(String(doc.get("status") || "").toLowerCase())).length;
  if (transactions.status === "fulfilled") depositsMinor = transactions.value.docs.filter((doc) => doc.get("type") === "deposit" && doc.get("currency") === "NGN").reduce((total, doc) => total + Number(doc.get("deltaMinor") || 0), 0);
  if (wallets.status === "fulfilled") walletMinor = wallets.value.docs.filter((doc) => String(doc.get("currency") || "NGN") === "NGN").reduce((total, doc) => total + Number(doc.get("availableMinor") ?? doc.get("balanceMinor") ?? 0), 0);
  if (growth.status === "fulfilled") customers = growth.value;
  const cards = [["Total customers", customers.total.toLocaleString("en-NG")], ["Joined today", customers.today.toLocaleString("en-NG")], ["Joined in 7 days", customers.week.toLocaleString("en-NG")], [`${periods[period]} growth`, `${customers.growth >= 0 ? "+" : ""}${customers.growth.toFixed(1)}%`], ["Active services", activeServices.toLocaleString("en-NG")], ["Orders requiring attention", pending.toLocaleString("en-NG")], ["Customer wallet balance", formatMoney(BigInt(walletMinor), "NGN")], ["Verified deposits", formatMoney(BigInt(depositsMinor), "NGN")]];
