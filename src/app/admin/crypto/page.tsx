...
export default async function CryptoAdminPage() {
  await requireAdmin();
  ...
  return <AppShell admin>
    ...
    <div className="grid3">
      ...
      <article className="glass card stat-card"><span className="muted">Verified</span><strong className="stat-value">{snapshot.docs.filter((doc) => doc.get("status") === "verified_pending_approval").length}</strong></article>
      <article className="glass card stat-card"><span className="muted">Approved in view</span><strong className="stat-value">{snapshot.docs.filter((doc) => doc.get("status") === "approved").length}</strong></article>
    </div>
    <div style={{ display: "grid", gap: 16, marginTop: 22 }}>
      {snapshot.empty ? <div className="glass card"><h2>No crypto payments yet</h2><p className="muted">Customer crypto submissions will appear here.</p></div> : snapshot.docs.map((doc) => {
        const item = doc.data(), status = String(item.status || "pending"), terminal = ["approved", "rejected", "cancelled"].includes(status), user = users.get(String(item.userId));
        return <article className="glass card" key={doc.id}>
          <div className="section-head"><div><span className="eyebrow">{String(item.network || "").replaceAll("_", " ")}</span><h2 style={{ marginBottom: 6 }}>{formatMoney(BigInt(Number(item.requestedNgnMinor || 0)), "NGN")}</h2><p className="muted" style={{ margin: 0 }}>{user?.name || "Customer"} · {user?.email || item.userId}</p></div><span className="status-pill">{status.replaceAll("_", " ")}</span></div>
