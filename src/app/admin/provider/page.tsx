...
export default async function ProviderHealthPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const health = await Promise.all(providers.map(async (provider) => {
    ...
    const [balance, services] = await Promise.all([
      ...
      provider.client.services().catch(() => null),
    ]);
    return { ...provider, connected: Boolean(balance && services), balance, serviceCount: services?.length || 0 };
  }));
  ...
  return <AppShell admin>
    ...
    <div className="grid3">{health.map((provider) => <article className="glass card stat-card" key={provider.key}><span className="eyebrow">{provider.label}</span><strong className="stat-value" style={{ fontSize: 28 }}>{provider.connected ? "Connected" : provider.configured ? "Action required" : "Not configured"}</strong><span className="muted">{provider.balance ? `${provider.balance.currency} ${provider.balance.balance} · ` : ""}{provider.serviceCount.toLocaleString("en-NG")} services</span></article>)}</div>
