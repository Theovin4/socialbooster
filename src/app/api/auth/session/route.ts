...
function trustedOrigin(origin:string|null){if(!origin)return false;try{const url=new URL(origin),app=new URL(process.env.NEXT_PUBLIC_APP_URL||"https://socialbooster.net.ng"),host=normalizedHost(url.hostname),allowed=new Set([normalizedHost(app.hostname),"socialbooster.net.ng"]);return (url.protocol==="https:"&&allowed.has(host))||url.hostname==="localhost"}catch{return false}}
...
export async function POST(request:Request){
  ...
  }catch(error){console.error("[auth:session] session creation failed",{error:error instanceof Error?error.message:"Unknown error"});return Response.json({error:"Authentication failed"},{status:401})}
}
