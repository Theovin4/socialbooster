...
export async function POST(request: Request) {
  ...
  const input = schema.parse(await request.json()), reference = `flw_${randomUUID()}`, amountMinor = Number(decimalToMinor(input.amount)), app = process.env.NEXT_PUBLIC_APP_URL || "https://www.socialbooster.net.ng";
