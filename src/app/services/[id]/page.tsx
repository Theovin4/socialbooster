...
export default async function ServiceDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const schema = { "@context": "https://schema.org", "@graph": [{ "@type": "Service", name: service.name, description: `Online ${service.categoryName} service with clear order limits and naira pricing.`, serviceType: service.categoryName, areaServed: [{ "@type": "Country", name: "Nigeria" }, { "@type": "Continent", name: "Africa" }], provider: { "@id": `${siteUrl}/#organization` }, offers: { "@type": "Offer", price: (sellingRateMinor / 100).toFixed(2), priceCurrency: "NGN", url: `${siteUrl}/services/${id}`, availability: "https://schema.org/InStock" } }, { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: `${siteUrl}/` }, { "@type": "ListItem", position: 2, name: "Services", item: `${siteUrl}/services` }, { "@type": "ListItem", position: 3, name: service.name, item: `${siteUrl}/services/${id}` }] }] };
