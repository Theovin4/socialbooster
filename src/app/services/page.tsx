...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    robots: q.trim() || Number(page) > 1 ? { index: false, follow: true } : { index: true, follow: true },
...
export default async function Services({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  const { q = "", page: pageInput = "1" } = await searchParams;
  const requestedPage = Math.max(1, Number.parseInt(pageInput, 10) || 1);
  let services: Service[] = [];
  let page = requestedPage, total = 0, totalPages = 1, pageSize = 50;
  let unavailable = false;
  try {
    const result = await getServiceCatalogPage({ query: q, page: requestedPage, pageSize });
    services = result.items.map((item) => ({ id: item.id, name: item.name, category: item.category, min: item.min, max: item.max, refill: item.refill, rate: item.rateMinor }));
    ({ page, total, totalPages, pageSize } = result);
  } catch (error) {
  ...
  return <><SiteHeader /><main className="shell" style={{ minHeight: "68vh", padding: "78px 0" }}>
    ...
    {unavailable ? <div className="glass card"><h2>Catalog temporarily unavailable</h2><p className="muted">Please check again shortly.</p></div> : services.length === 0 ? <div className="glass card"><h2>No services found</h2><p className="muted">Try a different search.</p></div> : <>
      <p className="muted">Showing {((page - 1) * pageSize + 1).toLocaleString("en-NG")}–{Math.min(page * pageSize, total).toLocaleString("en-NG")} of {total.toLocaleString("en-NG")} matching services</p>
      ...
      {totalPages > 1 ? <nav aria-label="Service catalogue pages" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginTop: 26 }}><span className="muted">Page {page.toLocaleString("en-NG")} of {totalPages.toLocaleString("en-NG")}</span><div style={{ display: "flex", gap: 10 }}>{page > 1 ? <Link className="btn" href={pageHref(page - 1)}>Previous</Link> : null}{page < totalPages ? <Link className="btn primary" href={pageHref(page + 1)}>Next</Link> : null}</div></nav> : null}
    </>}
  </main><SiteFooter /></>;
}
