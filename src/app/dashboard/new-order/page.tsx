...
export default async function NewOrderPage({ searchParams }: { searchParams: Promise<{ service?: string }> }) {
  ...
  return <AppShell><span className="eyebrow">Secure NGN order</span><h1 style={{ fontSize: 42 }}>New order</h1><div className="glass card"><form action={submitOrder} style={{ display: "grid", gap: 14 }}><label>Service<select className="field" name="serviceId" defaultValue={selected} required><option value="">Select a service</option>{snapshot.docs.map((doc) => { const item = doc.data(), price = serviceSellingRateNgnMinor(item); return <option value={doc.id} key={doc.id}>{item.name} — {formatMoney(price, "NGN")}/1,000</option>; })}</select></label><label>Target URL<input className="field" name="link" type="url" placeholder="https://..." required /></label><label>Quantity<input className="field" name="quantity" type="number" min="1" required /></label><label style={{ display: "flex", gap: 10 }}><input type="checkbox" name="confirmed" value="yes" required /> I confirm the target and quantity are correct and permitted.</label><button className="btn primary">Place order in NGN</button></form></div></AppShell>;
}
