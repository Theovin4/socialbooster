...
export default async function WalletPage({ searchParams }: { searchParams: Promise<{ payment?: string }> }) {
  ...
  if (payment === "success" || payment === "pending") await reconcilePendingFlutterwavePayments(user.uid);
  const db = adminDb();
  const [wallet, transactionSnapshot, cryptoSnapshot] = await Promise.all([
    ensureWallet(user.uid, "NGN"),
    db.collection("walletTransactions").where("userId", "==", user.uid).orderBy("createdAt", "desc").limit(5).get(),
    db.collection("cryptoDeposits").where("userId", "==", user.uid).limit(10).get(),
  ]);
