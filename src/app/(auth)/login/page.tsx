...
export default async function Login({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  const initialNotice = notice === "verify-email" ? { kind: "success" as const, title: "Account created", message: "Check your email and click the activation link. After verification, return here to sign in." } : notice === "reset-sent" ? { kind: "success" as const, title: "Check your email", message: "If an account matches that address, a password-reset link has been sent." } : notice === "signed-out" ? { kind: "success" as const, title: "Signed out", message: "You have been signed out securely." } : undefined;
